...
\title{Corpas-Core: A Constitutional Architecture for Personal Agentic AI Systems}
...
The governance gateway, \texttt{govern()}, is implemented as a deterministic, ten-step pipeline through which every agent must pass before acting. It enforces hard invariants before scoring, detects adversarial manipulation patterns in inbound requests, and appends every decision to a cryptographically hashed, append-only ledger. The current implementation (Layer~0) is intentionally deterministic for auditability and safety; probabilistic reasoning layers are deferred to future work. The system is validated by 72 passing tests, including a dedicated prompt-injection suite, and requires no large language model calls at the governance layer.

This paper presents the architecture, its design rationale, a formal objective function under bounded satisficing \citep{simon1956rational}, an evaluation protocol for longitudinal N-of-1 deployment \citep{lillie2011nof1}, an ablation analysis of the governance components, and a comparative assessment against existing personal AI approaches. Quantitative results from sustained deployment are forthcoming; the present contribution is the architecture, its formalisation, and the evaluation framework by which it will be assessed.

\end{abstract}

\noindent\textbf{Keywords:} personal AI systems; agentic architectures; retrieval-augmented generation; constitutional AI; multi-agent systems; cognitive operating systems; strategic decision-making; memory hierarchies; governance gateway; bounded satisficing
...
Personal AI systems are proposed as a remedy. The dominant implementation pattern is shallow: a chat interface backed by retrieval-augmented generation. These systems fail to align with the user's long-term mission, drift from the user's professional voice over time, and lack any principled mechanism for self-improvement. They are locally helpful but globally incoherent, optimising for the immediate query at the expense of the user's sustained trajectory.

This paper presents Corpas-Core, an architecture designed to function as a personal cognitive operating system. The system is governed by a machine-readable constitution that specifies decision rules, voice constraints, memory policy, agent roles, and immutable invariants. A bounded reflection protocol permits evolution of the system's behaviour, but only within a defined revisable layer. This applies the constitutional AI pattern introduced by \citet{bai2022constitutional} at the individual user level, rather than at the training-time alignment level for which it was originally designed.

This version extends the original system description with five additions: (i) a concrete, tested governance gateway; (ii) a formal objective function under bounded satisficing; (iii) an evaluation protocol for N-of-1 deployment; (iv) an ablation analysis of the governance components; and (v) a comparative assessment against existing approaches in the personal AI space.
...
\citet{bai2022constitutional} demonstrated that a set of written principles can constrain model behaviour during training, producing outputs that are more helpful and less harmful without requiring human feedback on every example. This is a powerful pattern, but it has not yet been applied systematically at the user level. Corpas-Core adopts and extends this pattern: the constitution is not a training-time artefact but a runtime document that governs agent behaviour, decision rules, and self-revision boundaries.
...
\begin{table}[H]
\centering
\caption{Comparative assessment of personal AI approaches across five dimensions.}
\label{tab:comparison}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Dimension} & \textbf{General-purpose LLM} & \textbf{RAG-only} & \textbf{Corpas-Core} \\
\midrule
Strategic alignment   & None          & None          & Constitutional rules \\
Memory quality        & Session only  & Flat vector   & Four-tier hierarchy  \\
Voice preservation    & None          & None          & Identity auditor     \\
Decision consistency  & Stochastic    & Stochastic    & Deterministic gateway \\
Adversarial robustness & None         & None          & Manipulation detection \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{R3, Prototype bias}: analysis-heavy language patterns (e.g., ``comprehensive review,'' ``full assessment'') trigger redirection to a prototype-first alternative.

\textbf{R4, Compounding}: one-shot patterns (tasks with no downstream reuse) are detected and downgraded in priority.

\textbf{R5, Anti-fragility}: six dimensions of fragility are scored (single point of failure, external dependency, reversibility, learning potential, optionality, time sensitivity). High fragility scores trigger rejection.
...
\begin{equation}
\mathcal{A}^* = \{ a \in \mathcal{A} \mid I(a) = \emptyset \;\wedge\; R(a) = \textsc{pass} \;\wedge\; L(a) \geq \tau_L \}
\end{equation}
...
The constitution is at version 0.2.0. The governance gateway is fully implemented as a deterministic pipeline. The codebase comprises 9 modules totalling approximately 800 lines of code. The test suite contains 72 tests that execute in under 2 seconds. Source code is available at \url{https://github.com/manuelcorpas/corpas-core}.
...
\begin{enumerate}
  ...
  \item \textbf{Self-deception risk.} A single-user system calibrated to the user's stated preferences may reinforce the user's blind spots rather than challenging them. The Critic agent mitigates this partially, but the constitutional principles themselves reflect the user's values.
...
Corpas-Core differs from these systems in its focus on long-term alignment with a single user's mission, its deterministic governance layer, and its explicit separation of immutable principles from revisable preferences. The bounded satisficing framework draws on \citet{simon1956rational}, and the Goodhart audit mechanism is informed by \citet{goodhart1975problems} and \citet{strathern1997improving}. The N-of-1 evaluation protocol follows the design principles articulated by \citet{lillie2011nof1}.
